        ...
        for idx, uploaded_file in enumerate(uploaded_files):
          try:
            ...
            if len(parts) >= 6:
              ...
              sport = (
                  parts[1]
                  if parts[1] in ["Baseball", "Football", "Basketball"]
                  else "Baseball"
              )
              team = parts[2]
...
with tab_vault:
  ...
  if not rows:
    ...
    for r in rows:
      ...
      card_list.append({
          "id": r[0],
          "player": r[1] if r[1] else "Unknown",
          "sport": r[2] if r[2] else "Baseball",
          "team": r[3] if r[3] else "Unknown",
          "year": int(r[4]) if r[4] else 2024,
          "card_number": r[5] if r[5] else "",
          "card_type": r[6] if r[6] else "Base",
          "val_low": v_low,
          "val_high": v_high,
          "front_path": r[9] if len(r) > 9 and r[9] else "",
          "date_added": r[10] if len(r) > 10 and r[10] else "",
      })
    ...
    for i in range(0, len(filtered), cols_per_row):
      ...
      for idx, card in enumerate(row_cards):
        with cols[idx]:
          ...
          st.markdown(f"### {card['player']} `({card['card_number']})`")
          st.caption(
